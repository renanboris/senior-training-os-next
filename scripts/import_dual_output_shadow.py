"""Script de importação de shadow exports para KnownSkills.
...
def build_dynamic_queries(skills: list[dict]) -> list[tuple[str, str]]:
    ...
    for goal_type, semantic_target in candidates:
        if (goal_type.lower(), semantic_target.lower()) in available:
            preferred.append((goal_type, semantic_target))
    if preferred:
        return preferred
    seen = set()
    fallback = []
...
def try_project_integration(skills: list[dict]) -> None:
    ...
    try:
        ...
        print(f"Planner: {planned.goal_type} / {planned.semantic_target}")
    except Exception as e:
...
def main() -> None:
    if len(sys.argv) < 2:
        print("Uso: python scripts/import_dual_output_shadow.py shadow_exports/ARQUIVO_shadow.jsonl")
        sys.exit(1)
    ...
    out_path = ROOT / "data" / "dual_output" / "imported_skills_from_shadow.jsonl"
